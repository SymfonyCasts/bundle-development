...
is what we want. Pass the `$object` instance. This retrieves the ID for the passed
object, regardless of how it's implemented.
...
Grab the first element of the array with `$id = reset($id)`. `dd()` that... and refresh the
browser. Perfect, just a single value!

Remove the `dd()` and down in our `findBy()` array, replace `$object->getId()` with
`$id`. Test it out in the browser... and it worked!
