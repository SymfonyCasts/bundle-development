...
Primero, en el directorio `src` de tu bundle, crea una nueva clase llamada `TranslatedObject`. Márcala como `final` y crea un constructor. Aceptará un único parámetro,`private object $_inner`. El guión bajo es una convención para garantizar que no haya conflictos al llamar a métodos o propiedades de esta envoltura.

## Añadir genéricos para mejorar la compatibilidad con el IDE

Vamos a utilizar de nuevo los genéricos de PHP, pero a nivel de clase. Añade un bloque doc de clase y añade `@template T of object`. A continuación, conviértelo en un mixin añadiendo `@mixin T`. Esto significa esencialmente que este objeto tendrá todos los mismos métodos y propiedades que el objeto de plantilla `T`.

A continuación, añade un bloque doc al constructor, y en lugar de `@param object`, sustitúyelo por `@param T`. Esto permite a PHPStorm saber que estamos inyectando aquí este objeto plantilla a nivel de clase.
...
`__call()` se llama cuando se utiliza un método que no existe en este objeto. Establece el tipo de retorno a `mixed`. Dentro, escribe`return $this->_inner->$name(...$arguments)`. Esto reenvía cualquier llamada a un método al objeto interior utilizando la variable `$name` como nombre del método. ¡PHP es así de guay!

`__get()` se invoca cuando se intenta acceder a una propiedad de este objeto que no existe. Establece el tipo de retorno en `mixed`, y dentro, reenvía al objeto interno con `return $this->_inner->$name`. De nuevo, utilizando la variable `$name` como nombre de la propiedad.
...
Recuerda que en nuestro método `ArticleController::show()`, estamos pasando el objeto artículo por este traductor y pasándoselo a Twig.
...
Se trata de un matiz de Twig que tendremos que tener en cuenta en nuestra envoltura. Lo abordaremos a continuación, ¡y con una prueba unitaria!
