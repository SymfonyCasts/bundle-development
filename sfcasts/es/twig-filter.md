...
A continuación, haz que extienda `AbstractExtension` de Twig. Ahora, anula un único método:`getFilters()`, y marca el tipo de retorno como `array`. Dentro, devuelve un array con un único elemento: `new TwigFilter()`. El primer argumento es el nombre de nuestro filtro,`translate_object`.
...
Ahora bien, si has escrito tiempos de ejecución Twig antes, puede que hayas creado una clase dedicada para ello, ¡pero esto no es estrictamente necesario! Puedes crear cualquier servicio como tiempo de ejecución con esta etiqueta. Luego, en tus extensiones, puedes hacer referencia a él como hemos hecho aquí.
