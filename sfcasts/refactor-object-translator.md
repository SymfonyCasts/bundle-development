...
it'll do for now. We'll mark this class as `final` and `@internal`. This way,
if we come up with a better name later, we can easily rename it.
...
`service('.symfonycasts.object_translator.mapping_manager')`. Love
that autocompletion!
...
`$builder->getDefinition('symfony_casts.object_translator.mapping_manager')`.
